...
from lib.dataset.idd_bi_strategy2_hrnet import idd_video_dataset, idd_video_dataset_PDA
from lib.dataset.utils import runningScore, transform_im_nor
...
def test():
    ...
    net = HighResolutionNet(n_classes=26)
    map_location = {'cuda:%d' % 2: 'cuda:%d' % 0}
    old_weight = torch.load(args.model_save_path, map_location=map_location)
    new_weight = {}
    ...
    with torch.no_grad():
         for step, sample in enumerate(test_data_loader):
            ...
            img = img_list[-1]
            print('-hrnet idd-', step, img.shape)
            pred = net(img.cuda())
