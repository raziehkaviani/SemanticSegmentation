...
class camvid_video_dataset(Dataset):
    ...
    #    print('-img-', idx, os.path.join(self.data_path, self.img_3_name[idx]))
        #print(img_3.shape)
        img_3 = transform_im(img_3)

#        print('----', os.path.join(self.gt_path, self.gt_label_name[idx]))
      ...
      #  self.img_2_name = []
        ...
        for i, line in enumerate(lines):
           ...
           # img_3_id = int(img_3_name[-22:-16])
            ...
            '''
            self.img_3_name.append(img_3_name)#[:-3]+'bmp')
            self.gt_label_name.append(gt_label_name)
         #   print('-img-', os.path.join(self.data_path, img_3_name[:-3]+'bmp'))
         #   img = cv2.imread(os.path.join(self.data_path, img_3_name[:-3]+'bmp'))
         #   print(img.shape)
        #    print('--', img_3_name[:-3]+'bmp', gt_label_name)
...
class camvid_video_dataset_PDA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_label = cv2.imread(os.path.join(self.gt_path, self.gt_label_name[idx]), 0)
        gt_label = torch.from_numpy(gt_label.astype(np.int64))
#        print('--', name, img.shape, gt_label.shape)
        return img, gt_label
    ...
    def get_list(self, list_path):
        ...
        for i, line in enumerate(lines):
          ...
          #      img_name = name.replace('{:06d}_leftImg8bit.png'.format(img_id),
            ...
            self.img_name.append(name)#[:-3]+'bmp')
