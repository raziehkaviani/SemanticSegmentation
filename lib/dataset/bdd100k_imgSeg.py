...
class bdd100k_video_dataset(Dataset):
    ...
    def __getitem__(self, idx):
       ...
       # img_2 = transform_im(img_2)
        ...
        if os.path.exists(os.path.join(self.data_path, self.img_3_name[idx])):
            img_3 = cv2.imread(os.path.join(self.data_path, self.img_3_name[idx]))
            img_3 = transform_im(img_3)
        else:
            print('image not found' + os.path.join(self.data_path, self.img_3_name[idx]))

        if os.path.exists(os.path.join(self.gt_path, self.gt_label_name[idx])):
            gt_label = cv2.imread(os.path.join(self.gt_path, self.gt_label_name[idx]), 0)
        else:
            print('GT not found' + os.path.join(self.gt_path, self.gt_label_name[idx]))

        if self.crop_size is not None:
            [img_3, gt_label] = randomcrop([img_3, gt_label], crop_size=self.crop_size)
    ...
    def get_list(self, list_path):
      ...
      #  self.img_2_name = []
        ...
        for i, line in enumerate(lines):
            img_3_name = line.split()[0]
            gt_label_name = os.path.join('labels/sem_seg/masks/train', img_3_name[-21:-3] + 'png')
            
            if os.path.exists(os.path.join(self.data_path, img_3_name)) and \
                os.path.exists(os.path.join(self.gt_path, gt_label_name)):
            ...
            # self.img_3_name.append(img_3_name)
            # self.gt_label_name.append(gt_label_name)
        print('--list train--', len(self.img_3_name), len(self.gt_label_name))


class bdd100k_video_dataset_PDA(Dataset):
    ...
    def get_list(self, list_path):
        ...
        for i, line in enumerate(lines):
            name = line.split()[0]
            gt_label_name = os.path.join('labels/sem_seg/masks/val', name[-21:-3] + 'png')

            if os.path.exists(os.path.join(self.data_path, name)) and \
                os.path.exists(os.path.join(self.gt_path, gt_label_name)):
          ...
          #      img_name = name.replace('{:06d}_leftImg8bit.png'.format(img_id),
            ...
            # self.img_name.append(name)
        print('--list val--', len(self.img_name))
